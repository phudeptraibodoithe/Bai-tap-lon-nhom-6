...
public class UserCommandHandler {
    ...
    public UserCommandHandler(ClientSession context) { this.context = context; }

    /*
     * Profile and money actions are tied to the logged-in client in context.
     * The client never sends an account name for these actions.
     */
    ...
    public void updateProfile(String raw) {
        ...
        boolean isSaved = userDAO.updateProfile(account, nickname, description, avatarURL, email, phone);
        context.sendResponse(new Response<>(ServerEvent.UPDATE_PROFILE.name(),
                isSaved ? ServerEvent.SUCCESS.name() : ServerEvent.ERROR.name(),
                isSaved ? "Cập nhật thành công" : "Lỗi cập nhật", null));
    }
    ...
    public void transaction(String raw) {
        ...
        try (Connection connection = DatabaseConnection.getConnection()) {
            boolean isPaid = userDAO.updateBalance(connection, context.getClientId(), amount);
            context.sendResponse(new Response<>(
                    ServerEvent.TRANSACTION.name(),
                    isPaid ? ServerEvent.SUCCESS.name() : ServerEvent.FAILED.name(),
                    isPaid ? "Giao dịch đã được xử lý!"
