...
class ClientContextTest {
    ...
    void testSendResponse_NullOut_ShouldNotThrow() {
        ClientContext noOut = new ClientContext();
        assertDoesNotThrow(
    ...
    void testGson_SerializesLocalDateTime() {
        assertDoesNotThrow(() -> {
            String json = ClientContext.gson().toJson(java.time.LocalDateTime.now());
            assertNotNull(json);
