...
import javafx.fxml.FXMLLoader; // Thêm thư viện để load FXML
import javafx.fxml.Initializable;
import javafx.scene.Parent;    // Thêm thư viện Parent
import javafx.scene.Scene;     // Thêm thư viện Scene
import javafx.scene.control.Button;
import javafx.scene.control.TableCell;
import javafx.scene.control.TableColumn;
import javafx.scene.control.TableView;
import javafx.scene.control.cell.PropertyValueFactory;
import javafx.stage.Stage;     // Thêm thư viện Stage

import java.io.IOException;    // Thêm thư viện bắt lỗi
import java.net.URL;
...
public class MyItemsController extends BaseController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        itemList = FXCollections.observableArrayList(
                new MyItem("SP001", "MacBook Pro M4 2026", "Người đăng", 35000000, "Đang diễn ra"),
                new MyItem("SP002", "Chuột Logitech MX Master 3", "Người đăng", 1500000, "Chờ duyệt"),
                new MyItem("SP005", "iPhone 16 Pro Max", "Người đấu giá", 28500000, "Đang dẫn đầu"),
                new MyItem("SP008", "Đồng hồ Apple Watch Series 10", "Người đấu giá", 9000000, "Bị vượt giá")
        );
        ...
        colAction.setCellFactory(param -> new TableCell<>() {
...

            {
                ...
                btnView.setOnAction(event -> {
                    ...
                    try {
                        // 1. Load file giao diện item.fxml (đảm bảo file nằm đúng trong src/main/resources)
                        FXMLLoader loader = new FXMLLoader(getClass().getResource("/item.fxml"));
                        Parent root = loader.load();

                        // 2. Lấy Controller của trang chi tiết và truyền dữ liệu sản phẩm sang
                        ItemController controller = loader.getController();
                        if (controller != null) {
                            // Gọi hàm truyền dữ liệu bên ItemController
                            controller.setItemData(selectedItem);
                        }

                        // 3. Thực hiện chuyển Scene
                        Stage stage = (Stage) btnView.getScene().getWindow();
                        stage.setScene(new Scene(root));
                        stage.show();

                    } catch (IOException e) {
                        System.err.println("Lỗi: Không tìm thấy file /item.fxml hoặc file bị lỗi cấu trúc!");
                        e.printStackTrace();
                    }
                });
            }
