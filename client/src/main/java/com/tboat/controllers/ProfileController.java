...
public class ProfileController extends BaseController implements Initializable, SocketListener {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        String cached = DataCache.getInstance().get(ServerEvent.GET_PROFILE);
        if (cached != null) {
    ...
    public void onSave(ActionEvent e) {
        ...
        SocketHelper.sendRequest(ServerEvent.UPDATE_PROFILE, payload);
    }
    ...
    public void logout(ActionEvent e) {
        if (AlertUtils.showConfirmation("Xác nhận đăng xuất", "Bạn có chắc chắn muốn đăng xuất không?")) {
            SocketHelper.sendRequest(ServerEvent.LOGOUT);
    ...
    private boolean handleBroadcast(ServerEvent type) {
        if (type == ServerEvent.AUCTION_FINISHED) {
            SocketHelper.sendRequest(ServerEvent.GET_PROFILE, null);
            return true;
        }
        return false;
    }
